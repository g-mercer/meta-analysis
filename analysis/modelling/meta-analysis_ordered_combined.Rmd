...
No algaecides. Test of moderators without algaecides for RVE is now significant. Test of Moderators (coefficients 1:3):¹ F(df1 = 3, df2 = 14.86) = 5.1722, p-val = 0.0120, indicating that we can reject the null hypothesis that pesticide class has a null effect on the difference between ai and formulation toxicity.
...
predictions_year_cen_plot <- ggplot() +
                                     geom_point(data = full_data_red_no_sim, aes(x = YEAR_PUBLISHED, y = LRR)) +
...
I want to impute the missing SEs, calculate the vi and fit the final model 101 times to see how sampling from the estimated gamma distribution affects the model estimates.
...
when using the imputed data select the median model in terms of herbicide estimate. Impute 101 models the 51st is selected.
...
ii) Imputed and non-imputed data without glyphosate

both imputed and non-imputed datasets have same patterns of results without glyphosate
...
orchard_plot(non_imputed_glyphosate_model,
             xlab = "Effect size lnRR", 
             group = "STUDY_ID",  k = TRUE, g = TRUE, trunk.size = 1.5)
...
v) Only look at end use liquid treatments.
...
ii) Heterogeneity. I had to run these checks on a model with a simplified random structure as the package orchaRd didn't take autoregressive correlation structures. This section is there to give a rough approximation of the heterogeneity
...
Another sensitivity analysis to perform is to filter only the CIs calculated by probit, perform an inverse probit transformation to get symmetrical CIs and perform the analysis again (extracting SE, calculate vi, fit final model).
...
# how many of each species
sort(table(full_data_red_no_sim$FORMULATION_TYPE),decreasing=TRUE)
```
